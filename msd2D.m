...
filenames = dir (['Corrected_',sample,'*.xls']);
% filenames = dir (['Corrected_*.xls']);


for c = 1:size(filenames,1)   %%% c will be 1 to the number of cells (csv files) which have to be processed  %%%  
    ...
    num_of_tracks = (size(telo_pos_xls,2)-1)/3
    FPS=(size(telo_pos_xls,1)-2)/(duration*60)
    time_lapse=1000/FPS
    
    position(1:size(telo_pos_xls,1)-2,1)=0:size(telo_pos_xls,1)-3; %writes frame number in 1st column
    
    for count_track=1:num_of_tracks
    for count_frame=1:size(telo_pos_xls,1)-2
        position(count_frame,count_track*2)=(telo_pos_xls{count_frame+2,(count_track*3)-1}); %fills the x coordinates 
        position(count_frame,(count_track*2)+1)=(telo_pos_xls{count_frame+2,count_track*3}); %fills the y coordinates 
    end
    end
...
%    xlswrite(filename , MSD_c) ;
   ...
   MSD_by_Tau_c=MSD_c;
    for count=2:size(MSD_by_Tau_c,2)
        MSD_by_Tau_c(3:end,count)=MSD_by_Tau_c(3:end,count)./MSD_by_Tau_c(3:end,1);
    end
%    xlswrite(['MSD by Tau_',sample,'_',num2str(c),'.xls'] , MSD_by_Tau_c) ;
...
xlswrite(['MSD by Tau_',sample,'_all.xlsx'] , MSD_by_Tau) ;